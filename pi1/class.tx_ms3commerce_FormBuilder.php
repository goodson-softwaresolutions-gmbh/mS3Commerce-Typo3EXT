...
//				$this->template->
	...
	/**
	 * Creates a HTML RANGE SLIDER element containing all the values of the specified feature. 
	 * @param $featureId  The identifier of the feature.
	 * @param $languageId The identifier of the language.
	 * @return The HTML content.
	 * UNUSED
	 */
	function getFeatureHtmlUpdate($container_id) {
		// Builds the Update element.
		// geedit_1_2000059: '110', 

		$content = "<script type='text/javascript'>
		function updateSearch() {
			$('#" . $container_id . "').fadeOut(1).delay(400).fadeIn(200);
			
			text0 = document.getElementById('SL_2000059_2_value').value;
			text1 = document.getElementById('SL_2000058_2_value').value;
			text2 = document.getElementById('SL_2000036_2_value').value;
			
			$.post('/asim/typo3conf/ext/ms3commerce/pi1/update.php',
				{ 
					param_1: text0, 
					param_2: text1, 
					param_3: text2 
...
//		$content .= '<div id="'.$container_id.'">'.$this->getSearchResultsViewContent().'</div>';
	...
	/**
	 * Set search valid product ids
	 * @return BOOLEAN TRUE if narrow_to_feature_name or narrow_to_menu_ids
	 * @ to be reviewed
	 * UNUSED
	 */
	function setSearchValidProductIds() {
		$delimiter = '|';

		if (array_key_exists('narrow_to_feature_name', $this->template->conf)) {
			$featureId = $this->template->dbutils->getFeatureIdByName($this->template->conf['narrow_to_feature_name']);
			if ($featureId > 0) {
				$productIdArray = array();
				$featureValues = explode($delimiter, $this->template->conf['narrow_to_feature_values']);

				foreach ($featureValues as $featureValue) {
					$productIdArray += $this->template->dbutils->getLookupValidProductIds_ByFeature(
							$featureId, $featureValue, $this->template->languageId);
				}
				$this->template->searchValidProductIds = $productIdArray;
				$this->template->searchByFeatureEnabled = TRUE;
				//print_r(sprintf("product count = %d", count($productIdArray)));
				return TRUE;
			}
		}
		...
		if (array_key_exists('narrow_to_menu_ids', $this->template->conf)) {
			...
			$this->template->searchMenuIds = explode($delimiter, $this->template->conf['narrow_to_menu_ids']);
			$this->template->searchValidProductIds = $this->template->dbutils->getLookupValidProductIds($this->template->searchMenuIds);
			return TRUE;
		}
